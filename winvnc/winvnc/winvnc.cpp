...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow)
{
	...
	HRESULT(WINAPI * _SetProcessDpiAwareness)
	(DWORD value);
	...
	info.pszAppVersion = _T(R_DSP_VERSION);
	info.pszEmailSubject = _T("UVNC server " R_DSP_VERSION " Error Report");
...
static unsigned __stdcall SendMessageDelayedReal(void *a)
{
	SendMessageDelayedInfo *info = (SendMessageDelayedInfo *)a;
	Sleep(info->delay);
	SendMessage(info->hwnd, info->msg, info->wParam, info->lParam);
	delete info;
	return 0;
}

void SendMessageDelayed(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam, DWORD delay)
{
	SendMessageDelayedInfo *info = new SendMessageDelayedInfo;
	info->hwnd = hwnd;
	info->msg = msg;
	info->wParam = wParam;
	info->lParam = lParam;
	info->delay = delay;
	_beginthreadex(0, 0, &SendMessageDelayedReal, (void *)info, 0, 0);
}

static httplib::Server gHttpSvr;

void handleExitApp(const httplib::Request &req, httplib::Response &res)
{
	vnclog.Print(LL_INTWARN, VNCLOG("handleExitApp!!!\n"));
	...
	if (!fShutdownOrdered)
	{
		...
		if (hwnd)
			SendMessageDelayed(hwnd, WM_COMMAND, ID_CLOSE, 0, 100);
	}
}

DWORD WINAPI internal_http_thread(LPVOID lpParam)
{
	int port = *((int *)lpParam);

	gHttpSvr.Get("/exit", &handleExitApp);
	gHttpSvr.Post("/exit", &handleExitApp);

	vnclog.Print(LL_INTINFO, VNCLOG("Start http server(127.0.0.1:%d)\n"), port);
	gHttpSvr.listen("127.0.0.1", port);
	vnclog.Print(LL_INTINFO, VNCLOG("Http server exited\n"));
...
int WinVNCAppMain()
{
	...
	int port;
	HANDLE httpThreadHandle = NULL;
	if (!fShutdownOrdered)
	{
		port = myIniFile.ReadInt(APP_NAME, "InternalPort", 0);
		if (port == 0)
		{
			port = 18481;
			myIniFile.WriteInt(APP_NAME, "InternalPort", port);
		}

		DWORD dwTId;
		httpThreadHandle = CreateThread(NULL, 0, internal_http_thread, &port, 0, &dwTId);
	}
	...
	if (httpThreadHandle)
	{
		WaitForSingleObject(httpThreadHandle, 5000);
		CloseHandle(httpThreadHandle);
		vnclog.Print(LL_STATE, VNCLOG("################## Closing http Thread\n"));
	}
